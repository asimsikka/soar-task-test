...
//   const { data, loading, error } = useQuery<Card[]>({ url: API_ENDPOINTS.CARD })
...
//       <div
//         key={card.number}
//         className={`min-h-56 w-full overflow-hidden rounded-[20px] p-6 transition-all duration-200 ${
//           isDark
...
//       <h1 className="mb-8 text-3xl font-bold text-gray-800">My Cards</h1>
//       <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
//         {data.map((card) => (
//           <div key={card.number} className="h-[400px]">
//             {renderCard(card)}
//           </div>
//         ))}
//       </div>
...
export default function CardsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:py-8">
      <h1 className="mb-6 sm:mb-8 text-2xl sm:text-3xl font-bold text-gray-800">My Cards</h1>
      <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
        {data.map((card) => (
          <div key={card.number} className="h-auto sm:h-[250px] md:h-[300px] lg:h-[250px]">
            {renderCard(card)}
          </div>
        ))}
      </div>
